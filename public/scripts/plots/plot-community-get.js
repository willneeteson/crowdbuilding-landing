...
function loadMorePosts() {
    ...
    // Get the current plot slug from the URL
    const pathParts = window.location.pathname.split('/');
    const plotSlug = pathParts[pathParts.length - 1] || pathParts[pathParts.length - 2];
    ...
    fetchPlotPosts(plotSlug, nextCursor);
}
...
async function toggleLike(postId) {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
        const data = await response.json();
        console.log('Like toggle full response:', JSON.stringify(data));
...
function renderPosts(posts, append = false) {
    ...
    posts.forEach(post => {
        ...
        console.log('Post', post.id, 'isLiked:', isLiked, 'likes_count:', post.likes_count, 'Likes:', JSON.stringify(post.likes));
...
async function deletePost(postId) {
    ...
    try {
        const response = await fetch(`https://api.crowdbuilding.com/api/v1/plots/${plotSlug}/posts/${postId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`
            }
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if (currentUserId) {
            ...
            // Call fetchPlotPosts with null cursor to fetch first page
            fetchPlotPosts(plotSlug, null);
